...
class NewOrder extends Component {
  ...
  componentDidMount() {
    ...
    if (this.props.orders.length) {
      ...
      const lastMonth = moment(this.props.orders[this.props.orders.length-1].date, 'DD.MM.YYYY').format('MM');
      const ordersInMonth = this.props.orders.filter(order => order.month === lastMonth).length;
      let id = '';
      ...
      this.setState({date, id, provider: this.providerInput.props.children[0].props.children});
    }
  }
